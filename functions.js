...
const channelHasVideo = (videoTitle, channel) =>
  channel.videos.some((channels) => videoTitle == channels.title);
...
const getChannelByName = (channelName, channels) =>
  channels.find((channelInfo) => channelName == channelInfo.name);
...
const searchChannels = (query, channels) =>
  channels.filter(
    (search) =>
      search.name.includes(query) || search.description.includes(query)
  );
